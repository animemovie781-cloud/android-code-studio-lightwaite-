...
fun Project.configureAndroidModule(coreLibDesugDep: Provider<MinimalExternalModuleDependency>) {
  ...
  extensions.getByType(BaseExtension::class.java).run {
    ...
    buildTypes.getByName("release") {
      ...
      // R8 is enabled ONLY for the main app module for APK size optimization
      isMinifyEnabled = project.plugins.hasPlugin("com.tom.rv2ide.core-app")
    ...
    buildTypes.register("dev") {
      ...
      // Explicitly disable resource shrinking since minification is off
      // (resource shrinking requires code shrinking to be enabled)
      if (isAppModule) {
